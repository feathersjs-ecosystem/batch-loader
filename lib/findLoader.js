const { stableStringify, defaultCacheParamsFn } = require('./utils')

module.exports = class FindLoader {
  constructor({ service, cacheParamsFn = defaultCacheParamsFn }) {
    this._options = { service }
    this._cacheParamsFn = cacheParamsFn
    this._cacheMap = new Map()
  }

  load(params, cacheParamsFn = this._cacheParamsFn) {
  ...
  clear(params, cacheParamsFn = this._cacheParamsFn) {
    const key = stableStringify([cacheParamsFn(params)])
  ...
  prime(params, data, cacheParamsFn = this._cacheParamsFn) {
    const key = stableStringify([cacheParamsFn(params)])
    const cached = this._cacheMap.get(key)
    if (cached) {
      return this
