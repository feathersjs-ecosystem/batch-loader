...
const { defaultCacheKeyFn } = require('./utils');

// TODO: There are duplicates here. These should be removed
// and docs/types updated. Would require a major version bump
const resultTypes = {
...
const DataLoader1 = module.exports = class DataLoader {
  constructor(batchLoadFn, options = {}) {
    if (typeof batchLoadFn !== 'function') {
      throw new Error([
        'DataLoader must be constructed with a function which accepts',
        `Array<key> and returns Promise<Array<value>>, but got: ${batchLoadFn}. (DataLoader)`
      ].join(' '));
    }
  ...
  load(key) {
    const { cacheKeyFn, batch, cache } = this._options;

    if (key === undefined) {
      throw new Error(
        `dataLoader.load() must be called with a value, but got: ${String(key)} (DataLoader).`
      );
    }

    if (Array.isArray(key)) {
      throw new Error(
        'dataLoader.load() called with an array. dataLoader.loadMany() must be used. (DataLoader).'
      );
    }

    const shouldBatch = batch !== false;
  ...
  loadMany(keys) {
    if (!Array.isArray(keys)) {
      throw new Error(
        `dataLoader.loadMany must be called with an array but got: ${keys}. (DataLoader)`
      );
    }
  ...
  clear(key) {
    const { cacheKeyFn } = this._options;
  ...
  static loaderFactory(service, id, multi, options = {}) {
    ...
    return context => new DataLoader(async (keys, context) => {
      const result = await service.find(makeCallingParams(
        context, { [id]: { $in: DataLoader1.getUniqueKeys(keys) } }, paramNames, injects
      ));

      return DataLoader1.getResultsByKey(keys, result, getKey, multi ? '[!]' : '!');
    },
      { context }
    );
  }
