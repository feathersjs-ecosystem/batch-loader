...
const createGetLoader = ({ service, loaderOptions }) => {
  if (!service.find) {
    throw new GeneralError('Cannot create a GetLoader for a service that does not have a find method.')
  }
...
module.exports = class ServiceLoader {
  ...
  load(id, params, cacheParamsFn = this._cacheParamsFn) {
    ...
    if (!idKey) {
      throw new GeneralError('Cannot call ServiceLoader.load on service that does not have an "options.id". You should configure the service id or you can call ServiceLoader.key("id").load(123) or ServiceLoader.exec({ id: 123, idKey: "id" })')
    }
